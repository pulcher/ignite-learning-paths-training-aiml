...
def convert(data):
    ...
    for key[0], value[0], confidence[0] in pageData['keyValuePairs'][0]:
        logging.info(f'key: {key} value: {value} confidence: {confidence}')
        # logging.info(f'items (key) (value.text): {items["key"]["text"]} {items["value"]["text"]}')

    logging.info(f'page[0]')
    ...
    keyPairs = { items["key"][0]["text"]: items["value"][0]["text"] if len(items["value"]) > 0 else "" 
                ...
                #  for items in pageData["page"][0]["keyValuePairs"] }

    tableItems = { col["header"][0]["text"]: [entry[0]["text"] for entry in col["entries"]]
                  for col in pageData["page"][0]["tables"][0]["columns"] }
