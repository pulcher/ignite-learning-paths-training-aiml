...
def convert(data):
    logging.info(f'data passed in: {data]')

    # get original data in a more normalized form 
    keyPairs = { items["key"][0]["text"]: items["value"][0]["text"] if len(items["value"]) > 0 else "" 
                 for items in data["page"][0]["keyValuePairs"] }
    logging.info(f'keyPairs: {keyPairs}')

    tableItems = { col["header"][0]["text"]: [entry[0]["text"] for entry in col["entries"]]
                  for col in data["page"][0]["tables"][0]["columns"] }
    logging.info(f'tableItems: {tableItems}')
    ...
    logging.info(limeitemCount = {lineItemcCount})
