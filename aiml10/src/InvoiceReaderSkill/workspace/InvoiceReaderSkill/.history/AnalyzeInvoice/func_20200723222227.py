    ...
    for record in body["values"]:
        try:
            ...
            if 'error' in cog_response:
                ...
                # cog_response_json = json.loads(cog_response)
                # logging.info(f'readResults = {cog_response_json['readResults']}')
                records['values'].append({
                    'recordId': record["recordId"],
                    'data': {
                        'formUrl': record["data"]["formUrl"],
                        'invoice': helpers.convert(cog_response['analyzeResult']['readResults']),
                        # 'invoice': helpers.convert(cog_response_json['readResults']),
                        'error': {}
                    }
                })
